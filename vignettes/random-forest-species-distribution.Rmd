...
The same interface can now be used for partial dependence and ICE plots.
`method = "pdp"` averages over sampled predictor combinations from `dat`,
whereas `method = "ice+pdp"` overlays that average on top of the sampled ICE
curves. Here `n = 30` controls both the grid resolution for numeric predictors
and the number of sampled rows used for PDP/ICE.
