...
sampled ICE curves. Here `n = 30` controls the grid resolution for numeric
predictors, while `background_n` controls how many background rows are
randomly sampled for PDP/ICE. For `method = "pdp"`, `interval = "quantile"`
can be used to show a central quantile band around the averaged curve.
...
The next examples follow the original workflow closely by plotting the second
